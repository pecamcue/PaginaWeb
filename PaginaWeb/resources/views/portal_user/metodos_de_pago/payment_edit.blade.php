...
<div class="container mt-5 pt-5">
    ...
    <h2 class="text-center fw-bold mb-4">Modificar Forma de <span class="text-primary">Pago</span></h2>
    ...
    <form action="{{ route('user.payment-methods.update', $paymentMethod->id) }}" method="POST" id="payment-form">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="card_type" class="form-label">Tipo de tarjeta</label>
            <select name="card_type" id="card_type" class="form-select" required>
                <option value="Visa" {{ $paymentMethod->card_type == 'Visa' ? 'selected' : '' }}>Visa</option>
                <option value="MasterCard" {{ $paymentMethod->card_type == 'MasterCard' ? 'selected' : '' }}>MasterCard</option>
                <option value="American Express" {{ $paymentMethod->card_type == 'American Express' ? 'selected' : '' }}>American Express</option>
            </select>
            @error('card_type')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="card_holder" id="card_holder" class="form-control" required value="{{ $paymentMethod->card_holder }}" autocomplete="off">
            @error('card_holder')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="expiry_date" id="expiry_date" class="form-control" required placeholder="MM/YY" maxlength="5" value="{{ $paymentMethod->expiry_date }}" autocomplete="off">
            @error('expiry_date')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
